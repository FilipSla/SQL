...
SELECT round(SUM(lat_n), 2) AS sumLat_N,
...
SELECT SUM(population) AS sumOfCaliforniaCities
FROM city
WHERE district = 'California';
/* Revising Aggregations - Averages */
SELECT AVG(population) AS averageCaliforniaCityPopulation
FROM city
WHERE district = 'California';
/* Average Population */
SELECT FLOOR(AVG(population)) AS averageCityPopulation
FROM city;
/* Japan Population */
SELECT SUM(population) AS japaneseCitiesPopulation
FROM city
WHERE countrycode = 'JPN';
...
SELECT truncate(SUM(lat_n), 4) AS suma
FROM station
WHERE lat_n BETWEEN 38.7880 AND 137.2345;
/* Weather Observation Station 14 */
SELECT MAX(truncate(lat_n, 4)) AS greatestNumber
FROM station
