SELECT
    ceil(weight / 10) * 10 AS weightGroup,
...
-----------------------
SELECT
    DISTINCT YEAR(birth_date) AS uniqueBirthYear
FROM
    patients
ORDER BY
    uniqueBirthYear ASC;
...
----------------------
SELECT
    concat(upper(last_name), ',', lower(first_name)) AS fullName
FROM
    patients
ORDER BY
    first_name DESC;
...
-----------------------
SELECT
    (MAX(weight) - MIN(weight)) AS diffWeight
FROM
    patients
WHERE
    last_name = 'Maroni';
